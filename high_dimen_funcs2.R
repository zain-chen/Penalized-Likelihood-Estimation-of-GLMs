...
simu.logi <- function(sig, real.beta)
{
  ...
  for(i in 1:200){
    ...
    x.name <- c()
    for(ooo in 1:p){
      x.name[ooo] <- paste("x", as.character(ooo))
    }
    colnames(x) <- x.name
    ...
    beta0.logi <- as.vector(glm(y.logi~x+0, family = binomial, maxit = 300)$coefficients)
    ...
    incorrect.mm[i] <- sum(abs(Logi.MM(beta0.logi, x, y.logi, opt.lam, thd1, thd2))[-c(1,2,3,4)]>0)
    ...
    incorrect.lqa[i] <- sum(abs(Logi.LQA(beta0.logi, x, y.logi, opt.lam, thd1, thd2))[-c(1,2,3,4)]>0)
    ...
    incorrect.glmnet[i] <- sum(abs(as.vector(lasso.fit$beta))[-c(1,2,3,4)]>thd1)
...
simu.pois <- function(sig, real.beta)
{
  ...
  for(i in 1:200){
    ...
    incorrect.mm[i] <- sum(abs(Pois.MM(beta0.pois, x, y.pois, opt.lam, thd1, thd2))[-c(1,2,3,4)]>0)
    ...
    incorrect.lqa[i] <- sum(abs(Pois.LQA(beta0.pois, x, y.pois, opt.lam, thd1, thd2))[-c(1,2,3,4)]>0)
